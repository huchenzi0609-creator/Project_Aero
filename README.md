...
双人（人机/联机）回合制策略网页游戏。灵感来自传统纸面游戏"海战"，双方在网格上秘密布置飞机，轮流报点，先击毁对方全部飞机者胜。整体视觉追求柔和纸张质感，支持横/竖版响应式布局（竖版固定 9:16 画幅）。
...
- 报点反馈：无飞机→击空；非机头部件→击中；机头→击毁（**被击毁飞机不公开位置**，其后对该机残骸报点一律按"无效打击"报击空）。
- 特殊规则·绝地反击：先手方一次行动击毁对方全部飞机且自身恰剩 1 架时，后手获一次额外报点；仅命中机头判后手胜，否则先手胜。
- 联机计时（围棋读秒制）：每回合默认 30 秒，超时消耗 1 次全局机会（共 3 次）并由系统代走一步；机会耗尽后每回合 10 秒；再超时机器接管。自定义房间可选每步 10/20/30/60 秒/不限。

## 当前版本特性（v0.2.x）

- 竖版 9:16 舞台（无滚动收容）；摆阵扑克牌叠放托盘；对局尺寸冻结
- 着色工具（黄/蓝/绿，点染/拖拽路径/批量染擦）；样式参考飞机可拖拽到棋盘辅助推演
- 全局浮窗"回到未完成的对局"；联机空网格外缘随回合变色；结算叠加双方标记与"平均击杀效率"统计
- 四级 AI：地狱=机头概率热图（斩首式报点），困难=全局热图+残骸建模
- 局域网部署：vite 已绑定全部网卡（host: true）
...
采用多 Agent 并行开发（队长负责对接与集成，UI/核心/后端/QA 分工）。见 `docs/design.md`。
