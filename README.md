...
- 超快棋：双方初始限时 10×n 秒（n=飞机架数），每成功报点 +1 秒，超时一方立即判负（忽略读秒制）。
- 盲棋：仅此模式允许对已报点格重复报点（残骸仍按击空）；对手网格只显示最近 3 个报点标记，击毁标记永存；禁用参考飞机拖放与着色工具。
- 预报点：非己方回合报点转为深红「?」预报点（至多 10 个，可取消）；轮到我方时按先进先出每回合自动上报一个。

## 当前版本特性（v0.3.0）

- 主菜单重构：新手教程 / 练习模式（经典、超快棋、盲棋、自定义）/ 对战模式（经典、超快棋、盲棋 + 自定义房间）
- 对战模式快速匹配：小/中/大档位多选勾选后一键匹配自动入局
- 超快棋（10n+1）：双方初始限时 10×n 秒（n=飞机架数）、每步 +1 秒、超时判负；<10 秒倒计时标红闪烁
- 盲棋：允许重复报点；对手网格仅显示最近 3 个报点标记（击毁标记永存）；禁用参考飞机拖放与着色工具
- 预报点：非己方回合报点转为深红「?」预报点（上限 10），轮到我方按 FIFO 每回合自动上报一个
- 快捷着色（默认开）：着色模式点击幽灵飞机 = 批量着色 + 退出着色模式 + 回收幽灵
- 新手教程：分步引导（摆阵 → 对战 → 实用工具），纸感对话气泡 + 遮罩突显 + 单元跳过
- 竖版 9:16 舞台（无滚动收容）；摆阵扑克牌叠放托盘；对局尺寸冻结
- 着色工具（黄/蓝/绿，点染/拖拽路径/批量染擦）；样式参考飞机可拖拽到棋盘辅助推演（幽灵飞机）
