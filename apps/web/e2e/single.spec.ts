...
test.describe('单机全流程', () => {
  // v0.3.13：教程套件与联机对局并行时，单机对局的 AI 定时器/回合成倍变慢；
  // v0.3.15 起报点序列改为「读会话快照取机头 → 少量实弹直取」，耗时回落到秒级。
  test.setTimeout(300_000)

  test('经典模式小型档：摆阵→横幅→对局→结算→再来一局', async ({ page }) => {
    const errs = watchErrors(page)
    await useE2eSeed(page) // v0.3.15：E2E_SEED → localStorage（AI 序列可复现）
    // 地狱 AI 更快结束对局：无论我方先手后手，双方任一方胜出都在更少步数内收敛
    ...
    // ---- 报点循环（v0.3.15 提速）：读会话快照取对手机头 → 2 发陪跑 + 3 个机头直取全歼；
    //      保留「双点报点（1/3）与输入框报点（2/3）交替」与「≥5 发」语义；快照不可得时回退对角门控循环。 ----
    ...
    if (heads.length >= 3) {
      ...
      // 回退：对角门控循环（无快照时仍收敛；保证不因对方回合空转堆积预报点）
      const order = diagCoords(10, 10)
      let idx = 0
      const deadline = Date.now() + 300_000
      while (Date.now() < deadline && !(await result.isVisible().catch(() => false))) {
        ...
        shotSet.add(coord)
        shotsTaken += 1
        await fireOnce(coord, shotsTaken % 3 === 0)
      }
    }
