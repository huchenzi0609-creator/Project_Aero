...
test.describe('主页', () => {
  test('四入口可见、用户名显示、教程弹窗、练习面板进出、横竖版切换', async ({ page }) => {
    ...
    // 版本角标 v0.3.0
    await expect(page.locator('.home__version')).toHaveText('v0.3.0')
    // 新手教程位于练习模式之上（y 序）
    ...
    // 新手教程：进入教程宿主页（入口弹窗 P1；G 教程全流程不在本 spec 范围）
    await page.getByRole('button', { name: '新手教程' }).click()
    await expect(page.locator('h1.page__title').filter({ hasText: '新手教程' })).toBeVisible()
    await expect(page.getByRole('dialog')).toContainText('您是否了解本游戏的基本规则？')
    await expect(page.getByRole('dialog')).toContainText('还没有')
    // 返回主页：结束教程并回 Home 面板
    await page.getByRole('dialog').getByRole('button', { name: '返回主页' }).click()
    await expect(page.getByRole('heading', { name: '飞机杀' })).toBeVisible()
