...
test.describe('主页', () => {
  test('四入口可见、用户名显示、教程弹窗、练习面板进出、横竖版切换', async ({ page }) => {
    ...
    // 版本角标 v0.3.18-beta5（release/v0.3.18；beta4 已被 revert，M8 在 beta3 基线重做）
    await expect(page.locator('.home__version')).toHaveText('v0.3.18-beta5')
    ...
    // 练习模式面板：四子模式入口可见；v0.3.17-beta5 练习模式小字说明 → 返回主页
    ...
    // v0.3.17-beta5：四张模式卡文案（自定义卡换新文案，其余三张回归保护）
    ...
    await expect(modeCards.filter({ hasText: '自定义模式' })).toContainText(
      '自定义棋盘尺寸，飞机形状、数量，限时与特殊规则。',
    )
    // 旧文案必须彻底不存在（'自定棋盘尺寸' = v0.3.17-beta4 的自定义卡文案，beta5 已改为
    // 「自定义棋盘尺寸，飞机形状、数量，限时与特殊规则。」，注意与旧片段的「自定」二字不同）
    for (const old of ['常规对局', '开局倒计时', '不记旧报点', '自定棋盘尺寸']) {
      await expect(
