...
## 7. v0.3.2 教程规则增补（最高优先级，覆盖上文冲突处）

1. **入口语义**：「我已了解」→ 直接进入教程·进阶（单元3）；「还不了解」→ 教程·基础（单元1）。
2. **多目标突显**：TutorialSpotlight 支持同时突显多个目标（如单元3 T3-3 同时突显【空网格】与【参考网格】）。
3. **弹窗豁免突显**：所有弹窗（.paper-modal__dialog 等教程弹窗）是"突显"遮罩的豁免对象——遮罩不得压暗/遮挡弹窗，弹窗始终完整可见可点。
4. **气泡持久性**：若某气泡被点击后会导致气泡消失（下一步为无文本的等待步），则点击不得使其消失（气泡保持显示直至事件推进），且该气泡不显示「点击继续」。
5. **胜利即时性**：教程·基础中我方玩家一旦击毁对方所有飞机（引擎 ended+winner=me），立即显示「恭喜！你获得了一场胜利！」，同时停止一切前置条件判定与前置文本显示（不再出现击空/击中反馈、击毁文本等）。
6. **继续对局收口**：教程·进阶「继续对局」后完成对局，结算页「返回主页」必须正确回到主页，不得进入"对局会话不存在"错误页（会话清理与导航收口由教程宿主统一处理）。

## 8. v0.3.4 教程规则增补（单元3 新时序，覆盖 §7 冲突处）

1. **染色轮询改 1 秒**：单元3「有方格被染色**或触发快捷着色**」→ 询问「1 秒内是否又有染色/快捷着色」→ 是：再等 1 秒重查；否：继续（原 3 秒窗口改 1 秒，且快捷着色事件也算活动）。
2. **T3-10 多目标突显**：显示工具教学气泡后，改为同时突显【空网格】与【着色工具按钮】（气泡不再突显）。
3. **T3-11 取消全部突显**：批量着色文本展示时（及之后）取消当前全部突显。
4. **T3-12 多目标突显**：创建预报点后，同时突显【对话气泡】与【空网格】显示预报点教学文本；文本展示后取消全部突显，等待 5 秒后对手 AI 恢复。
5. **退出收口（任意教程阶段）**：教程对局中点击对局内「退出」并「确认退出」必须直达主页，任何阶段（含非 free 教程期）都不得出现"对局会话不存在"错误页。
